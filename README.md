...
InvestSim은 업비트(Upbit) 거래소의 실시간 체결 데이터를 WebSocket으로 수신하여,
사용자가 직접 설계한 보조지표 기반 전략에 따라 모의 매수/매도를 자동 실행하는 백엔드 시스템입니다.

실제 자산 없이 전략의 수익성과 유효성을 검증할 수 있으며, 전체 시스템을 직접 설계하고 AWS에 배포하여 운영 중입니다.

### 개발 목표

단순 CRUD 수준을 넘어 실무에서 자주 마주치는 기술적 난제들을 직접 경험하고 해결하는 것을 목표로 기획했습니다.

- **실시간 데이터 처리**: WebSocket으로 수신한 체결 틱을 인메모리에서 즉시 처리하고, DB 접근은 매매 신호 발생 시로 최소화하여 부하를 제어
- **JPA 심화**: N+1 문제(`@EntityGraph`, `@BatchSize`), QueryDSL 동적 쿼리, 낙관적 락(`@Version`) 등 실제 도메인에서 발생하는 JPA 이슈를 직접 해결
- **아키텍처 설계**: 헥사고날 아키텍처를 적용해 Application 계층이 JPA나 WebSocket 같은 인프라 구현에 의존하지 않도록 설계하여, 변경 범위를 어댑터로 격리
- **보안 설계**: JWT Stateless 구조의 한계를 Redis 기반 Refresh Token Rotation과 Access Token 블랙리스트로 보완
